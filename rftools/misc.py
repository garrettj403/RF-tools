...
def effective_conductivity(freq, fermi_speed, e_density, mu_r=1):
    ...
    return (e_density ** 2 * e ** 4 / 
        (pi * mu_r * mu_0 * m_e ** 2 * fermi_speed ** 2 * f)) ** (1 / 3)
...
if __name__ == "__main__":
    ...
    print("")
    print("\tLength ratio at 300 K:\t\t{:5.1f}".format(mean_free_path(cond, vf, ne) / skin_depth(f, cond)))
    print("\tLength ratio at 4.2 K:\t\t{:5.1f}".format(mean_free_path(cond * rrr, vf, ne) / skin_depth(f, cond * rrr)))
    print("")
